...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Givecoin: starting difficulty is 1 / 2^12
...
{
    // Givecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 0;
    if (nHeight <= 5) {    // For Each 5 blocks will have 6k coins
       nSubsidy = 6000 * COIN;
    }
    else {
       nSubsidy = 1000 * COIN;
    }
    // Subsidy is cut in half every 172,500 blocks, which will occur approximately every 1 year
    nSubsidy >>= (nHeight / 172500); // Givecoin: 175.2k blocks in 1 years
    //
    if (nSubsidy < COIN) nSubsidy = COIN;  // Minimum Number of Coin = 1
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 24 * 60 * 60;                // Vimo: Difficulty adjusted every 10 days
static const int64 nTargetSpacing =   1 * 60;                          // Vimo: Every Minute
static const int64 nInterval = nTargetTimespan / nTargetSpacing;       // 10 block readjustment
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        int64  PastSecondsMin = TimeDaySeconds * 0.02; // 0.01;
        int64  PastSecondsMax = TimeDaySeconds * 0.30; // 0.14;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Givecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Givecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "Vimo or Virtual Money is born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("arkad4710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xd1, 0xd2, 0xd3, 0xdb }; // Givecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GivecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GivecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GivecoinMiner(CWallet *pwallet)
{
    printf("GivecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Givecoin-miner");
    ...
    try { loop {
        ...
        printf("Running GivecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GivecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GivecoinMiner, pwallet));
}
